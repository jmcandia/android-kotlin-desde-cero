...
Todas las funciones que has visto hasta ahora tienen un nombre. Pero Kotlin permite definir funciones **sin nombre**, que se llaman **funciones anónimas** o, en su forma más habitual, **lambdas**. Una lambda es, simplemente, una función que puedes tratar como un valor: guardarla en una variable, pasarla como argumento o devolverla desde otra función.

La sintaxis de una lambda usa llaves, flechas y el cuerpo de la función:

```kotlin
{ parámetros -> cuerpo }
```

Por ejemplo, aquí hay una lambda que suma dos números y la guardamos en una variable:
...
Fíjate en que no usamos `fun` ni le damos un nombre: la propia lambda es el valor. Cuando la invocas, usas el nombre de la variable como si fuera el nombre de una función.

Si la lambda tiene **un solo parámetro**, Kotlin te permite omitirlo y referirte a él con el nombre implícito `it`:

```kotlin
val duplicar = { numero: Int -> numero * 2 }

// Con 'it' (equivalente, cuando hay un único parámetro):
val duplicarConIt: (Int) -> Int = { it * 2 }

println(duplicar(5))       // 10
println(duplicarConIt(5))  // 10
```

> [!NOTE]Nota
> Al declarar `duplicarConIt`, especificamos el **tipo** de la variable como `(Int) -> Int`. Esto le dice a Kotlin que es una función que recibe un `Int` y devuelve un `Int`. Cuando el compilador puede inferir el tipo por contexto, no hace falta escribirlo; pero cuando la lambda va sola en una declaración con `it`, sí es necesario para que el compilador sepa de qué tipo es ese `it`.

## Funciones de orden superior

Una **función de orden superior** es una función que recibe otra función como parámetro, o que devuelve una función como resultado. Son posibles precisamente porque las lambdas son valores que se pueden pasar.

Veamos un ejemplo: una función `aplicarOperacion` que recibe dos números y una operación (una función), y la aplica:
...
El parámetro `operacion` tiene el tipo `(Int, Int) -> Int`: una función que recibe dos `Int` y devuelve un `Int`. Ahora podemos pasarle distintas lambdas según lo que necesitemos:

```kotlin
val suma    = aplicarOperacion(10, 3) { a, b -> a + b }
val resta   = aplicarOperacion(10, 3) { a, b -> a - b }
val producto = aplicarOperacion(10, 3) { a, b -> a * b }
...
> [!TIP]Sugerencia
> Cuando el último parámetro de una función es una lambda, Kotlin te permite escribirla **fuera de los paréntesis**, como en el ejemplo anterior. Esta convención se llama *trailing lambda* y hace que el código sea mucho más legible, sobre todo cuando la lambda ocupa varias líneas.

Las funciones de orden superior son una pieza clave de la programación funcional en Kotlin. Las encontrarás constantemente cuando trabajes con colecciones, que estudiaremos en la siguiente parte del curso: funciones como `filter`, `map` o `forEach` son funciones de orden superior que reciben una lambda para indicarles qué hacer con cada elemento.
...
- Las **lambdas** (funciones anónimas) son funciones sin nombre que se pueden guardar en variables y pasar como valores.
- Las **funciones de orden superior** reciben otras funciones como parámetros o las devuelven como resultado, lo que permite escribir código flexible y reutilizable.
